...
st.set_page_config(
    page_title="Real-Time Distributed Weather Data Analytics",
    layout="wide",
...
st.title("Real-Time Distributed Weather Analytics & Prediction")

mode = st.sidebar.radio("Data Engine", ["Real-Time Stream (Kafka/Spark v2)", "Global Historical (HDFS)"])
...
def load_stream_data():
    spark = get_spark()
    try:
        df = spark.read.parquet("hdfs://localhost:9000/weather-data/live_lake_v2")
        pdf = df.toPandas()
        
        # Parse all expected numeric columns
        ...
        # THE FIX: Calculate rolling micro-climate volatility dynamically in the UI
        # This prevents the Parquet/Spark sink limitation and ensures the UI always has the data.
        pdf["temp_volatility"] = pdf.groupby("city")["temperature"].transform(
            lambda x: x.rolling(window=5, min_periods=2).std()
        ).fillna(0.0)

        # Re-apply the heuristic flag
...
# ==========================================
if mode == "Real-Time Stream (Kafka/Spark v2)":
    st_autorefresh(interval=5000, key="data_refresh")
    df = load_stream_data()

    if df.empty:
        st.warning("Waiting for Kafka stream & Spark processor to write to HDFS v2...")
        st.stop()
    
    # --- 0. DYNAMIC ANOMALY STATUS SECTION ---
    # Replaced the red alert banner with a dedicated, permanent status section
    st.header("🛡️ System Health & Anomaly Monitor")
    ...
    if not recent_anomalies.empty:
        ...
        st.warning(f"⚠️ **STATUS: ANOMALY DETECTED** | High micro-climate volatility in: {affected_cities}")
    else:
        st.success("✅ **STATUS: GOOD** | All regional micro-climates are currently stable.")
    ...
    col_t4.metric("Engine Status", "Stateful Analytics Active")
    ...
    # --- 3. ADVANCED SPATIO-TEMPORAL TELEMETRY ---
    st.header("Advanced Spatio-Temporal Telemetry")
    col1, col2 = st.columns(2)
    
    # with col1:
    #     st.subheader("Live GBT PM2.5 Predictions")
    #     trend_df = city_df.tail(300)
        
    #     # Removed markers=True (they clutter smoothed lines)
    #     fig_trend = px.line(trend_df, x="timestamp", y="predicted_pm2_5", color="city")
        
    #     # THE FIX: Apply spline interpolation for smooth curves
    #     fig_trend.update_traces(line_shape='spline', line_smoothing=0.8)
        
    #     fig_trend.add_hline(y=15, line_dash="dash", line_color="red", annotation_text="WHO Safe Limit")
        
    #     # Optional formatting: Clean up the background and gridlines to make the colors pop
    ...
    with col1:
        st.subheader("Live GBT PM2.5 Distribution")
        trend_df = city_df.tail(300)
        
        # THE FIX: Switched to px.scatter to show data density instead of a continuous line
        fig_trend = px.scatter(
            trend_df, 
            x="timestamp", 
            y="predicted_pm2_5", 
            color="city", 
            opacity=0.6 # Low opacity makes overlapping dots look like a glowing cluster
        )
        
        # Polish: Make the dots a bit larger and give them a faint border for crispness
        fig_trend.update_traces(
            marker=dict(size=8, line=dict(width=1, color="rgba(255,255,255,0.2)"))
        )
    ...
    with col2:
        ...
        # This will now successfully render because temp_volatility is calculated
    ...
    if "latitude" in latest_data.columns and "longitude" in latest_data.columns:
        ...
        # This map will no longer crash because temp_volatility and is_anomaly are guaranteed to exist
        fig_map = px.scatter_mapbox(
            latest_data, lat="latitude", lon="longitude", size="marker_size", color="temperature",
            hover_name="city", hover_data=["predicted_pm2_5", "temp_volatility", "is_anomaly"], 
            zoom=1.5, height=400, color_continuous_scale="Inferno"
        )
        fig_map.update_layout(mapbox_style="carto-darkmatter")
        st.plotly_chart(fig_map, use_container_width=True)
    else:
        st.warning("Waiting for spatial features to propagate through the lake...")

# ==========================================
# GLOBAL DATASET LOGIC (Unchanged)
# ==========================================
else:
