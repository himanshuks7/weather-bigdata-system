...
1. `weather_producer.py` fetches weather from OpenWeatherMap for selected cities.
2. Producer sends enriched JSON events to Kafka topic `weather_stream_v3`.
3. `spark_stream_processorv3.py` consumes Kafka stream.
4. Spark groups events by city and keeps a rolling sequence buffer (length 10) using `applyInPandasWithState`.
5. Once buffer reaches 10 events, Spark applies scaler + traced PyTorch LSTM model to predict PM2.5 drift.
6. Results are written to HDFS path:
   - `hdfs://localhost:9000/weather-data/live_lake_v3`
7. `appv3.py` reads HDFS data and renders:
   - live telemetry
   - anomaly indicators
...
You requested `train_stm.py`; the repository file is currently `ml/train_lstm.py`.
This README documents `train_lstm.py`.
